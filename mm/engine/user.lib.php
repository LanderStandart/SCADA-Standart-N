...
function setmenu(){
  ...
  $this->html_menu=str_replace(":=user_caption=:",$this->fio,$this->html_menu);
}
...
<script type=\"text/javascript\" src=\"_res/ajax.js\"></script>";
  ...
  $this->html_content=str_replace(":availableTags:",file_get_contents("./13.txt"),$this->html_content);
  if (isset($_GET["query"])){
...
//   $q=ibase_query($this->default_trn,"select sname from wares ");
 
// while ($row=ibase_fetch_row($q)){

//   $AvailName=$row[0];
//   $AvailName=iconv('windows-1251', 'utf-8', $AvailName);
//   $this->AvailName=$this->AvailName.$AvailName;
// }
// }
}
?>