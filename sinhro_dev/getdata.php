...
$user->init();  
$db=$user->db;
$file_log='./logs/logFile.txt';
$now=$db->getOne('SELECT updatenow FROM UPDATE_TIME');
  $currentdate=new \DateTime (date ("Y-m-d H:i:s"));
      $currentdate->settimezone(new DateTimeZone('Europe/Samara'));

$last_log=date('Y-m-d H:i:s',filemtime($file_log));
$diff_log=($currentdate->diff($last_log));
if ($diff_log->hours>10) {
  copy($file_log,'./logs/logFile'.$last_log.'.txt');
...
function CheckStatus($url,$db,$name,$server_update,$idserv,$user){//Запрашиваем у сервера синронизации клиента его статус
  ...
  if (isset($xml->nameserver)){  
    ...
    $user->logFile('ALL OK DATA RECEIVE SUCCESS ');
}
      else{
        $user->logFile('not complite - ERROR- SERVER Unavailable');
...
//print_r($diffmin);
 ...
 $links = $db->query('SELECT url,id,nameserver from SERVERNAME where id='.$id);       
   while($row=mysqli_fetch_row($links)){
    ...
    if ($db->getone('SELECT ignore1 FROM SERVERNAME where id='.$id)==0) {CheckStatus($row[0],$db,$row[2],$server_update,$row[1],$user);}  
