...
echo '<tr  style="background-color: #d3dbe0">
  ...
  $malohod=R::getAll('SELECT caption,post_id,dt from MALOHOD');

  //while ($row=mysqli_fetch_row($malohod)) {
  foreach ($malohod as $row){
  	$dataUPD= new \Datetime ($row['dt']);
...
echo '<tr'.$color.' ><td>'.$row['post_id'].'</td> <td>'.$row['caption'].'</td>'; echo '<td>'.$dataUPD->format('d-m-Y H:i:s').'</td></tr>';  	
